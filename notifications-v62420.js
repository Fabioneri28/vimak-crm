/* VIMAK CRM V6.24.13.11.20 — Central Unificada de Notificações */
let vimakNotifState={
  rows:[],
  channel:null,
  startedFor:null,
...
// V6.24.13.11.20 • alertas operacionais calculados do cache, sem nova tabela/SQL
function vimakOperationalNotifications(){
 if(typeof vimakPriorityData!=='function')return [];
 return vimakPriorityData().map((x,i)=>({id:'op-'+i,type:'operational',severity:x.level==='critical'?'hot':'attention',title:`${x.tag} • ${x.title}`,message:`${x.sub||''}${x.value?' • '+x.value:''}`,created_at:new Date().toISOString(),read_at:null,hash:x.hash,_operational:true}));
}
function vimakAllNotifications(){return [...vimakOperationalNotifications(),...vimakNotifState.rows]}
function updateNotificationBadge(){
 const b=document.getElementById('notificationCount'),dot=document.getElementById('notificationDot');
 const n=vimakOperationalNotifications().length+vimakNotifUnread();
 if(b){b.textContent=n>99?'99+':String(n);b.hidden=n===0} if(dot)dot.classList.toggle('active',n>0)
}
function renderNotificationList(){
 const box=document.getElementById('notificationList');if(!box)return;const rows=vimakAllNotifications();
 box.innerHTML=rows.length?rows.map(x=>x._operational?`<button class="notif-row unread" onclick="closeModal();location.hash='${x.hash}'"><span class="notif-ico">${x.severity==='hot'?'!':'◇'}</span><span class="notif-copy"><b>${vimakNotifEsc(x.title)}</b><em>${vimakNotifEsc(x.message||'')}</em><small>Alerta operacional em tempo real</small></span></button>`:`<button class="notif-row ${x.read_at?'':'unread'}" onclick="markNotificationRead('${x.id}',true)"><span class="notif-ico">${notificationIcon(x)}</span><span class="notif-copy"><b>${vimakNotifEsc(x.title||'Novo lead')}</b><em>${vimakNotifEsc(x.message||'')}</em><small>${notificationTime(x.created_at)}</small></span>${x.whatsapp_status?`<span class="notif-wa ${x.whatsapp_status==='sent'?'ok':'warn'}">WA ${vimakNotifEsc(x.whatsapp_status)}</span>`:''}</button>`).join(''):'<div class="empty">Nenhuma notificação. Operação sob controle.</div>'
}
function openNotificationCenter(){
 const ops=vimakOperationalNotifications().length,leads=vimakNotifUnread();
 openModal('Central de Notificações',`<div class="notif-center"><div class="notif-head"><div><span>CENTRAL UNIFICADA</span><h3>${ops+leads} alerta(s) ativo(s)</h3><p>${ops} operacional(is) • ${leads} lead(s) não lido(s). Alertas operacionais desaparecem automaticamente quando a pendência é resolvida.</p></div><button class="btn" onclick="markAllNotificationsRead()">Ler notificações de leads</button></div><div id="notificationList" class="notif-list"></div></div>`,'');renderNotificationList()
}
