...
function ldSource(x){return x.source||x.canal_entrada||ldMeta(x).source||'Não informado'}
...
function ldActivities(id){return cache.leadActivities.filter(a=>a.lead_id===id)}
function ldTasks(id){return cache.leadTasks.filter(t=>t.lead_id===id)}
function ldLastContact(x){return x.last_contact_at||ldMeta(x).last_contact_at||ldActivities(x.id)[0]?.created_at||x.created_at}
function ldDays(v){return v?Math.max(0,Math.floor((Date.now()-new Date(v))/86400000)):999}
function ldScore(x){let manual=Number(x.score||ldMeta(x).score||0);if(manual)return Math.min(100,Math.max(0,manual));let s=20,i=leadStages.indexOf(ldStage(x));s+=Math.max(0,i)*9;if(ldPhone(x))s+=8;if(x.email)s+=5;if(ldValue(x))s+=8;if(['Quente','Muito quente'].includes(ldTemp(x)))s+=12;if(ldDays(ldLastContact(x))<=2)s+=8;if(ldDays(ldLastContact(x))>7)s-=12;return Math.min(100,Math.max(0,s))}
function ldScoreClass(v){return v>=75?'hot':v>=50?'warm':'cold'}
function ldSetView(v){leadView=v;render()}
function ldFiltered(){let q=leadSearch.toLowerCase();return cache.leads.filter(x=>(!q||[ldName(x),ldPhone(x),x.email,ldSource(x),ldOwner(x)].some(v=>String(v||'').toLowerCase().includes(q)))&&(leadOwner==='todos'||ldOwner(x)===leadOwner)&&(leadSource==='todos'||ldSource(x)===leadSource)&&(leadTemp==='todos'||ldTemp(x)===leadTemp))}

async function ldLoadExtras(){
  if(cache.leadExtrasLoaded)return;
  ...
  try{
    ...
    cache.leadExtrasError=err.message||'Execute a migration 007';
    console.warn('Lead extras:',err);
  }
}

function ldCard(x){let sc=ldScore(x),days=ldDays(ldLastContact(x));return `<article class="ld-card" onclick="ldView('${x.id}')"><div class="ld-card-top"><span class="ld-temp">${esc(ldTemp(x))}</span><b class="${ldScoreClass(sc)}">${sc}</b></div><h3>${esc(ldName(x))}</h3><p>${esc((x.environments||[]).join(', ')||ldMeta(x).project_type||'Projeto não definido')}</p><div class="ld-card-meta"><span>◉ ${esc(ldSource(x))}</span><span>◷ ${days===0?'Hoje':days+'d'}</span></div><div class="ld-card-value"><span>Potencial</span><b>${money(ldValue(x))}</b><small>${ldProbability(x)}% prob.</small></div></article>`}
...
function ldAnalytics(){let arr=cache.leads,active=arr.filter(x=>!['Pós-venda','Perdido','Cancelado'].includes(ldStage(x))),weighted=active.reduce((a,x)=>a+ldValue(x)*ldProbability(x)/100,0),sources=[...new Set(arr.map(ldSource))];return `<div class="ld-analytics-grid"><section class="card"><div class="ld-panel-head"><span>FUNIL COMERCIAL</span><h3>Distribuição por etapa</h3></div><div class="ld-funnel">${leadStages.map(st=>`<div><b>${st}</b><strong>${arr.filter(x=>ldStage(x)===st).length}</strong></div>`).join('')}</div></section><section class="card"><div class="ld-panel-head"><span>FORECAST PONDERADO</span><h3>${money(weighted)}</h3></div><div class="ld-source-list">${sources.map(src=>{let a=arr.filter(x=>ldSource(x)===src);return `<div><b>${esc(src)}</b><span>${a.length} leads</span><strong>${money(a.reduce((v,x)=>v+ldValue(x),0))}</strong></div>`}).join('')}</div></section><section class="card"><div class="ld-panel-head"><span>NEXT BEST ACTION</span><h3>Leads sem contato</h3></div>${active.filter(x=>ldDays(ldLastContact(x))>7).sort((a,b)=>ldScore(b)-ldScore(a)).slice(0,8).map(x=>`<button class="btn ld-alert" onclick="ldView('${x.id}')">${esc(ldName(x))} • ${ldDays(ldLastContact(x))}d</button>`).join('')||'<div class="empty">Nenhum lead crítico.</div>'}</section></div>`}
function ldList(){return `<div class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Lead</th><th>Etapa</th><th>Score</th><th>Origem</th><th>Responsável</th><th>Potencial</th><th></th></tr></thead><tbody>${ldFiltered().map(x=>`<tr><td><b>${esc(ldName(x))}</b><small>${esc(ldPhone(x))}</small></td><td>${esc(ldStage(x))}</td><td><b class="${ldScoreClass(ldScore(x))}">${ldScore(x)}</b></td><td>${esc(ldSource(x))}</td><td>${esc(ldOwner(x))}</td><td>${money(ldValue(x))}</td><td><button class="btn sm gold" onclick="ldView('${x.id}')">Abrir</button></td></tr>`).join('')||'<tr><td colspan="7" class="empty">Nenhum lead.</td></tr>'}</tbody></table></div></div>`}

function ldView(id){let x=cache.leads.find(v=>v.id===id);if(!x)return;let acts=ldActivities(id),tasks=ldTasks(id),sc=ldScore(x);openModal(ldName(x),`<div class="ld-detail"><div class="ld-detail-hero"><div><span class="ld-temp">${esc(ldTemp(x))}</span><h2>${esc(ldName(x))}</h2><p>${esc(ldPhone(x)||'Sem telefone')} • ${esc(x.email||'Sem e-mail')}</p></div><div class="ld-score-ring ${ldScoreClass(sc)}"><strong>${sc}</strong><span>LEAD SCORE</span></div></div><div class="ld-detail-kpis"><div><span>Etapa</span><b>${esc(ldStage(x))}</b></div><div><span>Potencial</span><b>${money(ldValue(x))}</b></div><div><span>Probabilidade</span><b>${ldProbability(x)}%</b></div><div><span>Forecast</span><b>${money(ldValue(x)*ldProbability(x)/100)}</b></div></div><div class="ld-detail-actions"><button class="btn gold" onclick="ldWhatsApp('${id}')">WhatsApp</button><button class="btn" onclick="ldAddActivity('${id}')">+ Interação</button><button class="btn" onclick="ldAddTask('${id}')">+ Próxima ação</button></div><div class="ld-detail-grid"><section><h3>Próximas ações</h3>${tasks.map(t=>`<div class="notice"><b>${esc(t.title)}</b><br>${new Date(t.due_at).toLocaleString('pt-BR')}</div>`).join('')||'<div class="empty">Sem tarefas.</div>'}</section><section><h3>Histórico comercial</h3>${acts.map(a=>`<div class="notice"><b>${esc(a.title||a.type)}</b><br>${esc(a.description||'')}</div>`).join('')||'<div class="empty">Sem interações.</div>'}</section></div></div>`,'')}
...
function ldAddActivity(id){if(cache.leadExtrasError)return toast('Execute a migration 007');openModal('Registrar Interação',`<div class="field"><label>Resumo</label><textarea id="ldaDesc"></textarea></div>`,`ldSaveActivity('${id}')`)}
...
function ldAddTask(id){if(cache.leadExtrasError)return toast('Execute a migration 007');openModal('Nova Próxima Ação',`<div class="form-grid"><div class="field full"><label>Tarefa</label><input id="ldtTitle"></div><div class="field"><label>Quando</label><input id="ldtDue" type="datetime-local"></div></div>`,`ldSaveTask('${id}')`)}
...
function leads(){
 ...
 let active=cache.leads.filter(x=>!['Pós-venda','Perdido','Cancelado'].includes(ldStage(x))),pipeline=active.reduce((a,x)=>a+ldValue(x),0),weighted=active.reduce((a,x)=>a+ldValue(x)*ldProbability(x)/100,0),hot=active.filter(x=>ldScore(x)>=75).length,stale=active.filter(x=>ldDays(ldLastContact(x))>7).length,owners=[...new Set(cache.leads.map(ldOwner))],sources=[...new Set(cache.leads.map(ldSource))];
 return shell('Leads & CRM Intelligence','Pipeline, Lead Score, follow-up, forecast e inteligência comercial 360°',`<button class="btn" onclick="ldSetView('analytics')">◈ Inteligência</button><button class="btn gold" onclick="addLead()">+ Novo Lead</button>`,`<div class="ld-command"><span class="measurement-version">V6.20 • SALES INTELLIGENCE</span><h2>Central Comercial VIMAK</h2><p>Do primeiro contato ao fechamento, carregando os recursos avançados somente quando este módulo é aberto.</p></div>${cache.leadExtrasError?`<div class="notice">Recursos de histórico/tarefas aguardam a migration 007. O restante do CRM continua normal.</div>`:''}<div class="grid g4 proposal-kpis"><div class="card kpi"><label>Pipeline ativo</label><strong>${money(pipeline)}</strong></div><div class="card kpi"><label>Forecast</label><strong class="goldtxt">${money(weighted)}</strong></div><div class="card kpi"><label>Leads quentes</label><strong class="green">${hot}</strong></div><div class="card kpi"><label>Sem contato +7d</label><strong class="${stale?'red':''}">${stale}</strong></div></div><div class="ld-toolbar"><div class="ld-views"><button class="${leadView==='pipeline'?'active':''}" onclick="ldSetView('pipeline')">▦ Pipeline</button><button class="${leadView==='analytics'?'active':''}" onclick="ldSetView('analytics')">◈ Inteligência</button><button class="${leadView==='lista'?'active':''}" onclick="ldSetView('lista')">☷ Lista</button></div><div class="ld-filters"><input placeholder="Buscar lead..." oninput="leadSearch=this.value;render()"><select onchange="leadOwner=this.value;render()"><option value="todos">Todos responsáveis</option>${owners.map(v=>`<option>${esc(v)}</option>`).join('')}</select><select onchange="leadSource=this.value;render()"><option value="todos">Todas origens</option>${sources.map(v=>`<option>${esc(v)}</option>`).join('')}</select></div></div>${leadView==='pipeline'?ldPipeline():leadView==='analytics'?ldAnalytics():ldList()}`)}
...
let cmTab='cockpit';
async function cmLoadExtras(){
 if(cache.cardExtrasLoaded)return;
 cache.cardExtrasLoaded=true;
 try{
   const [r,t,sx]=await Promise.all([
   ...
   if(r.error||t.error||sx.error)throw new Error(r.error?.message||t.error?.message||sx.error?.message);
   cache.cardMachineRates=r.data||[];cache.cardTransactions=t.data||[];cache.cardSettlements=sx.data||[];cache.cardExtrasError='';
   if(page==='maquininhas')render();
 }catch(err){cache.cardExtrasError=err.message||'Execute a migration 006';console.warn('Card extras:',err)}
}
function cmProvider(m){return m?.provider||m?.brand||m?.name||'Maquininha'}
function cmRates(id){return cache.cardMachineRates.filter(r=>r.card_machine_id===id&&r.active!==false)}
function cmBest(amount,type,inst,brand='Todas'){return cache.cardMachines.filter(m=>m.active!==false).map(m=>{let rs=cmRates(m.id).filter(r=>r.payment_type===type&&Number(r.installments||1)===Number(inst)&&(!r.card_brand||r.card_brand===brand||r.card_brand==='Todas'));let rate=rs.sort((a,b)=>Number(a.rate_pct)-Number(b.rate_pct))[0];if(!rate)return null;let pct=Number(rate.rate_pct||0),fixed=Number(rate.fixed_fee||0),fee=amount*pct/100+fixed;return {m,rate,pct,fee,net:amount-fee}}).filter(Boolean).sort((a,b)=>b.net-a.net)}
function cmSetTab(v){cmTab=v;render()}
function cmCockpit(){let gross=cache.cardTransactions.reduce((a,x)=>a+Number(x.gross_amount||0),0),fees=cache.cardTransactions.reduce((a,x)=>a+Number(x.fee_amount||0),0),net=gross-fees,pending=cache.cardSettlements.filter(x=>x.status!=='Conciliado').reduce((a,x)=>a+Number(x.net_amount||0),0);return `<div class="grid g4 proposal-kpis"><div class="card kpi"><label>Volume processado</label><strong>${money(gross)}</strong></div><div class="card kpi"><label>Taxas</label><strong class="goldtxt">${money(fees)}</strong></div><div class="card kpi"><label>Líquido</label><strong class="green">${money(net)}</strong></div><div class="card kpi"><label>A receber</label><strong>${money(pending)}</strong></div></div><div class="cm-grid"><section class="card"><h3>Adquirentes cadastradas</h3>${cache.cardMachines.map(m=>`<div class="cm-line"><b>${esc(cmProvider(m))}</b><span>${cmRates(m.id).length} taxas cadastradas</span></div>`).join('')||'<div class="empty">Nenhuma maquininha cadastrada.</div>'}</section><section class="card"><h3>Motor de decisão</h3><p class="cm-copy">O ranking usa somente as taxas reais cadastradas para a VIMAK. Nada de tabela genérica.</p><button class="btn gold" onclick="cmSetTab('simulador')">Abrir simulador</button></section></div>`}
function cmSimulator(){return `<div class="card"><h3>Simulador inteligente</h3><div class="form-grid"><div class="field"><label>Valor</label><input id="cmsAmount" type="number" value="10000"></div><div class="field"><label>Modalidade</label><select id="cmsType"><option>Débito</option><option selected>Crédito</option><option>Pix</option></select></div><div class="field"><label>Parcelas</label><select id="cmsInst">${Array.from({length:12},(_,i)=>`<option ${i===9?'selected':''}>${i+1}</option>`).join('')}</select></div><div class="field"><label>Bandeira</label><select id="cmsBrand"><option>Todas</option><option>Visa/Mastercard</option><option>Elo/Amex</option></select></div></div><button class="btn gold" onclick="cmRunSim()">Comparar maquininhas</button><div id="cmSimOut"></div></div>`}
function cmRunSim(){let amount=Number(cmsAmount.value||0),best=cmBest(amount,cmsType.value,Number(cmsInst.value),cmsBrand.value);cmSimOut.innerHTML=`<div class="cm-ranking">${best.map((x,i)=>`<article class="${i===0?'winner':''}"><b>${i===0?'★ ':''}${esc(cmProvider(x.m))}</b><span>${x.pct.toFixed(3)}%</span><span>Taxa ${money(x.fee)}</span><strong>Líquido ${money(x.net)}</strong></article>`).join('')||'<div class="notice">Cadastre a taxa dessa combinação na Tabela de Taxas.</div>'}</div>`}
function cmMachines(){return `<div class="cm-machine-grid">${cache.cardMachines.map(m=>`<article class="card"><h3>${esc(cmProvider(m))}</h3><p>${esc(m.model||m.name||'')}</p><span>${cmRates(m.id).length} taxas</span></article>`).join('')||'<div class="card empty">Nenhuma maquininha cadastrada.</div>'}</div>`}
function cmRatesView(){return `<div class="card"><div class="cm-head"><h3>Tabela de Taxas Contratadas</h3><button class="btn gold" onclick="cmNewRate()">+ Nova Taxa</button></div><div class="table-wrap"><table class="table"><thead><tr><th>Adquirente</th><th>Modalidade</th><th>Bandeira</th><th>Parcelas</th><th>Taxa</th><th>Prazo</th></tr></thead><tbody>${cache.cardMachineRates.map(r=>`<tr><td>${esc(cmProvider(cache.cardMachines.find(m=>m.id===r.card_machine_id)))}</td><td>${esc(r.payment_type)}</td><td>${esc(r.card_brand||'Todas')}</td><td>${r.installments||1}x</td><td><b>${Number(r.rate_pct||0).toFixed(3)}%</b></td><td>D+${r.settlement_days||0}</td></tr>`).join('')||'<tr><td colspan="6" class="empty">Nenhuma taxa cadastrada.</td></tr>'}</tbody></table></div></div>`}
function cmNewRate(){if(cache.cardExtrasError)return toast('Execute a migration 006');openModal('Nova Taxa',`<div class="form-grid"><div class="field"><label>Maquininha</label><select id="cmrMachine">${cache.cardMachines.map(m=>`<option value="${m.id}">${esc(cmProvider(m))}</option>`).join('')}</select></div><div class="field"><label>Modalidade</label><select id="cmrType"><option>Débito</option><option>Crédito</option><option>Pix</option></select></div><div class="field"><label>Parcelas</label><select id="cmrInst">${Array.from({length:12},(_,i)=>`<option>${i+1}</option>`).join('')}</select></div><div class="field"><label>Taxa %</label><input id="cmrRate" type="number" step=".001"></div><div class="field"><label>Prazo D+</label><input id="cmrDays" type="number" value="1"></div></div>`,`cmSaveRate()`)}
async function cmSaveRate(){let r=await sb.from('card_machine_rates').insert({company_id:profile.company_id,card_machine_id:cmrMachine.value,payment_type:cmrType.value,card_brand:'Todas',installments:Number(cmrInst.value),rate_pct:Number(cmrRate.value||0),settlement_days:Number(cmrDays.value||0),active:true});if(r.error)return toast('Erro: '+r.error.message);closeModal();cache.cardExtrasLoaded=false;await cmLoadExtras()}
function maquininhas(){if(!cache.cardExtrasLoaded)setTimeout(cmLoadExtras,0);let tabs=[['cockpit','◈ Cockpit'],['simulador','◎ Simulador'],['maquinas','▣ Maquininhas'],['taxas','% Taxas']];return shell('Maquininhas & Taxas PRO','InfinitePay, Mercado Pago, Leozinha e qualquer adquirente',`<button class="btn gold" onclick="cmSetTab('simulador')">◎ Simular</button>`,`<div class="cm-command"><span class="measurement-version">V6.19 • PAYMENT ACQUIRING CONTROL TOWER</span><h2>Central Inteligente de Maquininhas</h2><p>Escolha a melhor rota de pagamento pelas condições reais negociadas pela VIMAK.</p></div>${cache.cardExtrasError?`<div class="notice">Recursos avançados aguardam a migration 006. O restante do CRM continua normal.</div>`:''}<div class="cm-tabs">${tabs.map(t=>`<button class="${cmTab===t[0]?'active':''}" onclick="cmSetTab('${t[0]}')">${t[1]}</button>`).join('')}</div>${cmTab==='cockpit'?cmCockpit():cmTab==='simulador'?cmSimulator():cmTab==='maquinas'?cmMachines():cmRatesView()}`)}
