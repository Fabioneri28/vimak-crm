...
function ldPhone(x){return x.phone||x.telefone||''} function ldName(x){return x.name||x.nome||'Lead'}
...
async function ldDrop(ev,stage){let id=ev.dataTransfer.getData('text/plain');if(!id)return;let r=await sb.from('leads').update({stage,status:stage,updated_at:new Date().toISOString()}).eq('id',id);if(r.error)return toast('Erro: '+r.error.message);await ldLog(id,'Mudança de etapa',`Lead movido para ${stage}`,'stage');toast('Lead movido');render()}
