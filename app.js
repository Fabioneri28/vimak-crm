...
async function inputImportVimakBase(){if(!confirm(`Importar/atualizar ${VIMAK_STOCK_SEED.length} produtos da base VIMAK? Produtos com o mesmo SKU serão atualizados.`))return;toast('Importando base VIMAK...');let ok=0,fail=0;for(const r of VIMAK_STOCK_SEED){const payload={...r,company_id:profile.company_id,active:true,photo_url:VIMAK_STOCK_ACTUAL_IMAGES[r.sku]||`assets/stock/${r.sku}.svg`};delete payload.legacy_photo;const {error}=await sb.from('inputs').upsert(payload,{onConflict:'company_id,sku'});if(error)fail++;else ok++}await persistRefresh(`Base importada: ${ok} itens${fail?` • ${fail} falhas`:''}`)}
...
function insumos(){
 ...
 `<div class="row-actions"><button class="btn" onclick="inputHelp()">? Como usar</button><button class="btn" onclick="inputImportVimakBase()">⇧ Importar base VIMAK</button><button class="btn gold" onclick="addInput()">+ Cadastrar produto</button></div>`,
