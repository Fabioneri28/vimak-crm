...
["PROPOSTAS",[["insumos","▥","Insumos"],["propostas","▤","Propostas"],["contratos","✒","Contratos"],["orcamentopdf","📐","Orçamento por PDF"],["modelos","▥","Modelos de Proposta"],["medicoes","⌗","Medições"],["compras","▰","Compras"]]],
  ...
  const rows=cache.proposals.map(x=>{
    ...
    return `<tr>
      ...
      <td><div class="row-actions">
        ...
        <button class="btn sm" onclick="printProposal('${x.id}')">PDF</button><button class="btn sm gold" onclick="openContractFromProposal('${x.id}')">Contrato</button>
...
const CONTRACT_STATUS=['Rascunho','Gerado','Enviado','Aguardando assinatura','Assinado','Cancelado'];
function contractStoreKey(){return `vimak_contracts_${company?.id||'local'}`}
function contractLoad(){try{return JSON.parse(localStorage.getItem(contractStoreKey())||'[]')}catch(e){return []}}
function contractSaveAll(a){localStorage.setItem(contractStoreKey(),JSON.stringify(a))}
function contractByProposal(pid){return contractLoad().find(x=>x.proposal_id===pid)}
function contractNumber(p){const n=String(p?.number||p?.id||'').replace(/\D/g,'').slice(-4).padStart(4,'0');return `CT-${new Date().getFullYear()}-${n}`}
function contractClientAddress(c={}){return [c.address||c.street,c.number,c.complement,c.neighborhood,c.city,c.state,c.zip_code||c.zip].filter(Boolean).join(', ')}
function contractUpsert(pid,patch={}){let a=contractLoad(),i=a.findIndex(x=>x.proposal_id===pid),p=proposalById(pid);if(!p)return null;let x=i>=0?a[i]:{id:crypto.randomUUID?.()||String(Date.now()),proposal_id:pid,number:contractNumber(p),status:'Rascunho',created_at:new Date().toISOString()};x={...x,...patch,updated_at:new Date().toISOString()};if(i>=0)a[i]=x;else a.unshift(x);contractSaveAll(a);return x}
function contractStatusClass(s){return s==='Assinado'?'ok':s==='Cancelado'?'bad':s==='Enviado'||s==='Aguardando assinatura'?'blue':''}
function openContractFromProposal(pid){contractUpsert(pid);location.hash='contratos';page='contratos';render();setTimeout(()=>editContract(pid),250)}
function contractForm(pid){const p=proposalById(pid)||{},c=proposalClient(p.client_id)||{},x=contractByProposal(pid)||{};return `<div class="contract-editor"><div class="contract-banner"><div><span>${esc(x.number||contractNumber(p))}</span><h3>${esc(c.name||'Cliente')}</h3><p>${esc(p.title||'Proposta')}</p></div><span class="badge ${contractStatusClass(x.status)}">${esc(x.status||'Rascunho')}</span></div><div class="form-grid"><div class="field"><label>Status</label><select id="ctStatus">${CONTRACT_STATUS.map(v=>`<option ${v===(x.status||'Rascunho')?'selected':''}>${v}</option>`).join('')}</select></div><div class="field"><label>Prazo de fabricação / entrega (dias úteis)</label><input id="ctDays" type="number" value="${esc(x.delivery_days||p.delivery_days||50)}"></div><div class="field"><label>Entrada (R$)</label><input id="ctEntry" type="number" step="0.01" value="${Number(x.entry_value||0)}"></div><div class="field"><label>Saldo (R$)</label><input id="ctBalance" type="number" step="0.01" value="${Number(x.balance_value||Math.max(0,Number(p.total||0)-Number(x.entry_value||0)))}"></div><div class="field full"><label>Condições de pagamento</label><textarea id="ctPayment" rows="3">${esc(x.payment_terms||p.payment_terms||'')}</textarea></div><div class="field"><label>MDF interno</label><input id="ctInternal" value="${esc(x.mdf_internal||'BRANCO TX 15MM')}"></div><div class="field"><label>Portas / frentes</label><input id="ctDoors" value="${esc(x.mdf_doors||'MDF 18MM conforme projeto aprovado')}"></div><div class="field"><label>Corrediças</label><select id="ctSlides">${['Ocultas com amortecimento','Telescópicas','Conforme projeto'].map(v=>`<option ${v===(x.slides||'Conforme projeto')?'selected':''}>${v}</option>`).join('')}</select></div><div class="field"><label>Dobradiças</label><select id="ctHinges">${['Com amortecimento','Sem amortecimento','Conforme projeto'].map(v=>`<option ${v===(x.hinges||'Com amortecimento')?'selected':''}>${v}</option>`).join('')}</select></div><div class="field full"><label>Observações / especificações adicionais</label><textarea id="ctNotes" rows="4">${esc(x.notes||'')}</textarea></div></div><div class="contract-source-note"><b>Preenchimento automático</b><span>Cliente, CPF/CNPJ, endereço, proposta, valor, prazo e condições comerciais são reaproveitados do CRM. Revise antes de emitir.</span></div></div>`}
function editContract(pid){const p=proposalById(pid);if(!p)return toast('Proposta não encontrada');openModal('Contrato Inteligente VIMAK',contractForm(pid),`saveContract('${pid}')`);modal.classList.add('proposal-modal')}
function saveContract(pid){const p=proposalById(pid);if(!p)return;const entry=Number(document.getElementById('ctEntry')?.value||0);contractUpsert(pid,{status:document.getElementById('ctStatus')?.value||'Rascunho',delivery_days:Number(document.getElementById('ctDays')?.value||50),entry_value:entry,balance_value:Number(document.getElementById('ctBalance')?.value||Math.max(0,Number(p.total||0)-entry)),payment_terms:document.getElementById('ctPayment')?.value||'',mdf_internal:document.getElementById('ctInternal')?.value||'',mdf_doors:document.getElementById('ctDoors')?.value||'',slides:document.getElementById('ctSlides')?.value||'',hinges:document.getElementById('ctHinges')?.value||'',notes:document.getElementById('ctNotes')?.value||''});closeModal();render();toast('Contrato atualizado')}
function setContractStatus(pid,status){contractUpsert(pid,{status});render();toast('Status do contrato: '+status)}
function contractItemsText(pid){const items=proposalItems(pid);return items.length?items.map(i=>`${i.environment?i.environment+' — ':''}${i.description}`).join(' • '):'Móveis planejados conforme proposta e projeto aprovado.'}
function printContract(pid){const p=proposalById(pid);if(!p)return;let x=contractByProposal(pid)||contractUpsert(pid,{status:'Gerado'});if(x.status==='Rascunho'){x=contractUpsert(pid,{status:'Gerado'})}const c=proposalClient(p.client_id)||{},w=window.open('','_blank','width=980,height=760');if(!w)return toast('Permita pop-ups para gerar o PDF');const today=new Date().toLocaleDateString('pt-BR');const addr=contractClientAddress(c)||'—';const items=contractItemsText(pid);w.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${esc(x.number)} - Contrato</title><style>*{box-sizing:border-box}body{margin:0;background:#eee;font-family:Arial,sans-serif;color:#211f1b}.page{width:210mm;min-height:297mm;margin:10mm auto;background:#fff;padding:18mm 17mm}.head{display:flex;justify-content:space-between;border-bottom:3px solid #c9973f;padding-bottom:14px}.brand{font-size:25px;font-weight:900}.brand span{color:#c9973f}.num{text-align:right}.num b{display:block}.info{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:18px 0}.box{border:1px solid #ddd4c5;border-radius:8px;padding:10px;font-size:12px}.box b{display:block;margin-bottom:4px}h2{font-size:15px;margin:18px 0 7px;text-transform:uppercase}p,li{font-size:11.5px;line-height:1.55;text-align:justify}.sign{display:grid;grid-template-columns:1fr 1fr;gap:45px;margin-top:55px;text-align:center}.line{border-top:1px solid #333;padding-top:7px;font-size:11px}.foot{margin-top:30px;border-top:1px solid #ddd;padding-top:8px;font-size:9px;color:#777}@media print{body{background:#fff}.page{margin:0;box-shadow:none;page-break-after:auto}}</style></head><body><div class="page"><div class="head"><div><div class="brand">${esc(company.name||'VIMAK')} <span>PLANEJADOS</span></div><small>CONTRATO DE FABRICAÇÃO DE MÓVEIS</small></div><div class="num"><b>${esc(x.number)}</b><small>${today}</small></div></div><div class="info"><div class="box"><b>CONTRATADA</b>${esc(company.legal_name||company.name||'VIMAK Planejados')}<br>CNPJ: ${esc(company.document||company.cnpj||'50.996.037/0001-90')}</div><div class="box"><b>CONTRATANTE</b>${esc(c.name||'—')}<br>CPF/CNPJ: ${esc(c.document||c.cpf_cnpj||'—')}</div><div class="box"><b>ENDEREÇO DO CLIENTE / INSTALAÇÃO</b>${esc(addr)}</div><div class="box"><b>PROPOSTA VINCULADA</b>${esc(proposalNumber(p.number))} • ${esc(p.title||'')}<br>Valor: ${money(p.total)}</div></div><h2>1. Do objeto</h2><p>O presente contrato tem por objeto a fabricação e instalação de móveis planejados com predominância de madeira pela CONTRATADA, conforme proposta comercial, projeto aprovado, especificações e condições deste instrumento.</p><h2>2. Obrigações da contratante</h2><p>A CONTRATANTE fornecerá as informações necessárias ao desenvolvimento do produto, incluindo definições de cores, texturas, estilos e, quando aplicável, planta do imóvel e informações de instalações hidráulicas, elétricas e estruturais. Compromete-se ainda a cumprir as condições de pagamento acordadas.</p><h2>3. Obrigações da contratada</h2><p>A CONTRATADA executará os serviços conforme o descritivo e projeto aprovado, preservará os dados e documentos do cliente para finalidade contratual e fornecerá os documentos fiscais cabíveis.</p><h2>4. Escopo e especificações</h2><p><b>Ambientes/itens:</b> ${esc(items)}</p><p><b>MDF interno:</b> ${esc(x.mdf_internal||'BRANCO TX 15MM')}. <b>Portas/frentes:</b> ${esc(x.mdf_doors||'MDF 18MM conforme projeto aprovado')}. <b>Corrediças:</b> ${esc(x.slides||'Conforme projeto')}. <b>Dobradiças:</b> ${esc(x.hinges||'Com amortecimento')}.</p><h2>5. Preço e condições de pagamento</h2><p>Valor total contratado: <b>${money(p.total)}</b>. Entrada: <b>${money(x.entry_value||0)}</b>. Saldo: <b>${money(x.balance_value||Math.max(0,Number(p.total||0)-Number(x.entry_value||0)))}</b>. Condições: ${esc(x.payment_terms||p.payment_terms||'A combinar')}.</p><h2>6. Prazo</h2><p>O prazo previsto para fabricação/entrega é de <b>${Number(x.delivery_days||p.delivery_days||50)} dias úteis</b>, observadas a aprovação final do projeto, medição técnica e condições necessárias para execução.</p><h2>7. Multas, inadimplência e desistência</h2><p>O descumprimento contratual poderá ensejar multa de 10% sobre o valor do contrato. Em atraso de parcela, poderão incidir multa moratória de 2%, juros de 1% ao mês pro rata die e correção monetária por índice oficial. Em desistência, serão considerados os custos comprovadamente incorridos, materiais adquiridos, serviços executados e despesas operacionais, considerando a natureza sob medida dos móveis.</p><h2>8. Rescisão</h2><p>A rescisão deverá ser comunicada por escrito. Permanecem devidos os valores correspondentes às etapas executadas, materiais adquiridos e demais custos incorridos até a comunicação.</p><h2>9. LGPD</h2><p>Os dados pessoais necessários serão tratados para execução deste contrato, cumprimento de obrigações legais e demais bases legais aplicáveis da Lei Geral de Proteção de Dados.</p><h2>10. Condições do local de instalação</h2><p>O cliente deverá disponibilizar o ambiente limpo, desocupado e em condições adequadas de acesso e instalação. A impossibilidade de execução por condições do local poderá gerar reagendamento, custos adicionais e prorrogação de prazo.</p><h2>11. Foro</h2><p>Fica eleito o foro da Comarca de São Paulo/SP para dirimir controvérsias oriundas deste instrumento, ressalvadas as hipóteses legais aplicáveis.</p>${x.notes?`<h2>Observações adicionais</h2><p>${esc(x.notes)}</p>`:''}<p style="margin-top:28px">São Paulo, ${today}.</p><div class="sign"><div class="line">${esc(company.legal_name||company.name||'VIMAK Planejados')}<br>CONTRATADA</div><div class="line">${esc(c.name||'CONTRATANTE')}<br>CONTRATANTE</div></div><div class="foot">${esc(x.number)} • Documento gerado pelo VIMAK CRM a partir da proposta ${esc(proposalNumber(p.number))}. Revise os dados e condições antes da assinatura.</div></div><script>window.onload=()=>setTimeout(()=>window.print(),350)<\/script></body></html>`);w.document.close();render()}
function contratos(){const all=contractLoad();const signed=all.filter(x=>x.status==='Assinado').length,waiting=all.filter(x=>['Enviado','Aguardando assinatura'].includes(x.status)).length,total=all.reduce((a,x)=>a+Number(proposalById(x.proposal_id)?.total||0),0);const rows=cache.proposals.map(p=>{const c=proposalClient(p.client_id),x=contractByProposal(p.id);return `<tr><td><b>${esc(x?.number||contractNumber(p))}</b><small>${esc(proposalNumber(p.number))}</small></td><td><b>${esc(c?.name||'Cliente')}</b><small>${esc(p.title||'')}</small></td><td><b class="goldtxt">${money(p.total)}</b></td><td><span class="badge ${contractStatusClass(x?.status||'Rascunho')}">${esc(x?.status||'Rascunho')}</span></td><td><div class="row-actions"><button class="btn sm gold" onclick="editContract('${p.id}')">Configurar</button><button class="btn sm" onclick="printContract('${p.id}')">PDF</button>${x?.status!=='Assinado'?`<button class="btn sm" onclick="setContractStatus('${p.id}','Assinado')">Assinado</button>`:''}</div></td></tr>`}).join('');return shell('Contratos','Contratos inteligentes vinculados diretamente às propostas VIMAK',`<button class="btn" onclick="location.hash='propostas'">← Propostas</button>`,`<div class="contract-hero"><div><span>V6.24.14 • CONTRATOS INTELIGENTES</span><h2>Da proposta aprovada ao contrato em poucos cliques.</h2><p>Reaproveite dados do cliente, projeto, valor, pagamento e especificações sem redigitar.</p></div></div><div class="grid g4 proposal-kpis"><div class="card kpi"><label>Contratos gerados</label><strong>${all.length}</strong></div><div class="card kpi"><label>Aguardando assinatura</label><strong>${waiting}</strong></div><div class="card kpi"><label>Assinados</label><strong class="goldtxt">${signed}</strong></div><div class="card kpi"><label>Valor em propostas</label><strong>${money(total)}</strong></div></div><div class="filters"><div class="field"><label>Buscar</label><input placeholder="Contrato, cliente, proposta..." oninput="filterTable(this.value)"></div></div><div class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Contrato</th><th>Cliente / Projeto</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead><tbody id="rows">${rows||'<tr><td colspan="5" class="empty">Nenhuma proposta disponível.</td></tr>'}</tbody></table></div></div>`)}

const VIEWS={dashboard,leads,empresa,usuarios,auditoria,planos,clientes,fornecedores,parceiros,posvenda,insumos,propostas,contratos,orcamentopdf,modelos,medicoes,compras,templates,kanban,corte,sobras,cortecloud,equipes,agenda,financeiro,rentabilidade,custosequipe,maquininhas};
