...
async function stockImportConfirm(){const rows=stockImportState.rows;if(!rows.length)return;closeModal();toast('Importando produtos e imagens...');let ok=0,fail=0,photos=0;for(let i=0;i<rows.length;i++){const r=rows[i],img=stockImportImageFor(r);const payload={...r,company_id:profile.company_id};delete payload.legacy_photo;let {data,error}=await sb.from('inputs').upsert(payload,{onConflict:'company_id,sku'}).select('id').single();if(error){fail++;continue}ok++;if(img&&data?.id){try{const ext=(img.name.split('.').pop()||'jpg').toLowerCase();const blob=img.file||await img.entry.async('blob');const path=`${profile.company_id}/${data.id}-${Date.now()}.${ext}`;const {error:upErr}=await sb.storage.from('input-images').upload(path,blob,{contentType:blob.type||`image/${ext==='jpg'?'jpeg':ext}`,upsert:true});if(!upErr){const {data:u}=sb.storage.from('input-images').getPublicUrl(path);await sb.from('inputs').update({photo_url:u.publicUrl}).eq('id',data.id);photos++}}catch(_e){}}if(i%20===0)toast(`Importando... ${i+1}/${rows.length}`)}await persistRefresh(`Importação concluída: ${ok} produtos • ${photos} fotos${fail?` • ${fail} falhas`:''}`)}
...
function insumos(){
 ...
 `<div class="row-actions"><button class="btn" onclick="inputHelp()">? Como usar</button><button class="btn" onclick="inputImportVimakBase()">⇧ Importar CSV + fotos ZIP</button><button class="btn gold" onclick="addInput()">+ Cadastrar produto</button></div>`,
